...
/**
 * Logic for generating SUS Series Proposals
 * Added support for SUS113
 */
async function generateSUSProposal(btn) {
    ...
    try {
        ...
        doc.autoTable({
            ...
            body: [
                ...
                ['Permeate Water Flux', 'LMH', '10-40'], // SUS uses 10-40
        ...
        doc.autoTable({
            ...
            body: [
                ...
                ['Pore size', '0.1 Micron (outside - in) '], // SUS pore size
                ['Fiber Size (OD/ID)', '2.5mm / 1.9mm'], // SUS fiber size
...
/**
 * Logic for generating SUS Series Word Proposals
 */
async function generateSUSWordProposal(btn) {
    ...
    try {
        ...
        const membraneImgDataUrl = await loadImage('Images for Proposal/SUS_img.jpg');
        const pidImgDataUrl = await loadImage('Images for Proposal/MembraneP&ID.jpg');
        const gaImgDataUrl = await loadImage('Images for Proposal/SUS_GADrawing.png');
        ...
        const page1Children = [
            ...
            new Paragraph({ text: "" }),
            new Paragraph({
                 children: [new ImageRun({ data: membraneImgBuffer, transformation: { width: 400, height: 400 } })], // Adjusted for SUS image aspect ratio
                 alignment: AlignmentType.CENTER
            }),
            new Paragraph({ text: "" }),
            new Paragraph({
            ...
            new Paragraph({ text: "• Energy saving due to low operating pressure", indent: { left: 350 } }), // Corrected typo from BF
...
window.generateSUSProposal = generateSUSProposal;
window.generateSUSWordProposal = generateSUSWordProposal;
